...
forma en que se tiene acceso a los campos de las variables de tipo estructura tanto para asignacion de valores como para lectura y escritura. */
...
struct alumno                    /* DEclaracion de la estructura. */
{
    ...
    float promedio;
    char direccion[20];
 };           /* Observa que la declaracion de una estrsutura termina con punto y coma. */

void main(void)
{
    /* Observa que las variables de tipo estrustura se declaran como cualquier otra
    variable. a1, a2 y a3 son variables de tipo estrsuctua alumno. */
    ...
    a3.promedio = pro;
    fflush(stdin);
    printf("Ingrese la direcciosdn del alumno 3: ");
    gets(dir);
